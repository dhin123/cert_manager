...
def create_user():
    ...
    if validate_schema(data, USER_SCHEMA):
        response = requests.post('http://customer-service/internal_customer', json=data)
        ...
        return jsonify({
            "status": {
                "code": 400,
                "message": "Invalid schema. Please use the correct schema"
            }
        }), 400
...
def delete_user(customer_id):
    response = requests.delete(f'http://customer-service/internal_customer/{customer_id}')
    log.info(f"Received request to delete customer with id :{customer_id}")
    # Return the response received from the customer service
    return response.content, response.status_code
...
def create_certificate():
    ...
    if validate_schema(data, CERT_SCHEMA):
        response = requests.post('http://cert-service/internal-certificate', json=data)
        log.info(f"Received request to create certificate with:{data}")
        # Return the response received from the certificate service
        return response.content, response.status_code
    else:
        return jsonify({
            "status": {
...
def get_active_certificates(customer_id):
    response = requests.get(f'http://cert-service/internal-certificates/{customer_id}')
...
def change_certificate_status(certificate_id):
    ...
    if validate_schema(data, CERT_STATUS_SCHEMA):
        response = requests.patch(f'http://cert-service/internal-certificate/{certificate_id}', json=data)
